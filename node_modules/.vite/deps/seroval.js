...
function createStringNode(value) {
	return createSerovalNode(1, NIL, serializeString(value), NIL, NIL, NIL, NIL, NIL, NIL, NIL, NIL, NIL);
}
...
function createDateNode(id, current) {
	...
	return createSerovalNode(5, id, timestamp !== timestamp ? "" : current.toISOString(), NIL, NIL, NIL, NIL, NIL, NIL, NIL, NIL, NIL);
}
function createRegExpNode(id, current) {
	return createSerovalNode(6, id, NIL, serializeString(current.source), current.flags, NIL, NIL, NIL, NIL, NIL, NIL, NIL);
}
function createWKSymbolNode(id, current) {
	return createSerovalNode(17, id, INV_SYMBOL_REF[current], NIL, NIL, NIL, NIL, NIL, NIL, NIL, NIL, NIL);
}
function createReferenceNode(id, ref) {
	return createSerovalNode(18, id, serializeString(getReferenceID(ref)), NIL, NIL, NIL, NIL, NIL, NIL, NIL, NIL, NIL);
}
function createPluginNode(id, tag, value) {
	return createSerovalNode(25, id, value, serializeString(tag), NIL, NIL, NIL, NIL, NIL, NIL, NIL, NIL);
}
function createArrayNode(id, current, parsedItems) {
	return createSerovalNode(9, id, NIL, NIL, NIL, NIL, NIL, parsedItems, NIL, NIL, getObjectFlag(current), NIL);
}
...
function createTypedArrayNode(id, current, buffer) {
	return createSerovalNode(15, id, NIL, current.constructor.name, NIL, NIL, NIL, NIL, buffer, current.byteOffset, NIL, current.length);
}
function createBigIntTypedArrayNode(id, current, buffer) {
	return createSerovalNode(16, id, NIL, current.constructor.name, NIL, NIL, NIL, NIL, buffer, current.byteOffset, NIL, current.byteLength);
}
function createDataViewNode(id, current, buffer) {
	return createSerovalNode(20, id, NIL, NIL, NIL, NIL, NIL, NIL, buffer, current.byteOffset, NIL, current.byteLength);
}
function createErrorNode(id, current, options) {
	return createSerovalNode(13, id, getErrorConstructor(current), NIL, serializeString(current.message), options, NIL, NIL, NIL, NIL, NIL, NIL);
}
function createAggregateErrorNode(id, current, options) {
	return createSerovalNode(14, id, getErrorConstructor(current), NIL, serializeString(current.message), options, NIL, NIL, NIL, NIL, NIL, NIL);
}
...
function parseIteratorFactory(ctx) {
	...
	if (result.type === 1) return result.value;
	return createSerovalNode(27, result.value, NIL, NIL, NIL, NIL, NIL, NIL, parseWellKnownSymbol(ctx, SYM_ITERATOR), NIL, NIL, NIL);
}
function parseAsyncIteratorFactory(ctx) {
	const result = getNodeForIndexedValue(ctx, ASYNC_ITERATOR);
	if (result.type === 1) return result.value;
	return createSerovalNode(29, result.value, NIL, NIL, NIL, NIL, NIL, [parseSpecialReference(ctx, 1), parseWellKnownSymbol(ctx, SYM_ASYNC_ITERATOR)], NIL, NIL, NIL, NIL);
}
function createObjectNode(id, current, empty, record) {
	return createSerovalNode(empty ? 11 : 10, id, NIL, NIL, NIL, record, NIL, NIL, NIL, NIL, getObjectFlag(current), NIL);
}
function createMapNode(ctx, id, k, v) {
	return createSerovalNode(8, id, NIL, NIL, NIL, NIL, {
		k,
		v
	}, NIL, parseSpecialReference(ctx, 0), NIL, NIL, NIL);
}
function createPromiseConstructorNode(ctx, id, resolver) {
	return createSerovalNode(22, id, resolver, NIL, NIL, NIL, NIL, NIL, parseSpecialReference(ctx, 1), NIL, NIL, NIL);
}
function createArrayBufferNode(ctx, id, current) {
	...
	return createSerovalNode(19, id, serializeString(btoa(result)), NIL, NIL, NIL, NIL, NIL, parseSpecialReference(ctx, 5), NIL, NIL, NIL);
}
...
async function parseAggregateError(ctx, depth, id, current) {
	const options = getErrorOptions(current, ctx.base.features);
	return createAggregateErrorNode(id, current, options ? await parseProperties(ctx, depth, options) : NIL);
}
...
async function parsePromise(ctx, depth, id, current) {
	...
	return createSerovalNode(12, id, status, NIL, NIL, NIL, NIL, NIL, await parseAsync(ctx, depth, result), NIL, NIL, NIL);
}
...
async function parseObjectAsync(ctx, depth, id, current) {
	...
	switch (currentClass) {
		...
		case NIL: return parsePlainObject(ctx, depth, id, current, true);
...
function deserializeArray(ctx, depth, node) {
	...
	for (let i = 0, item; i < len; i++) {
		item = items[i];
		if (item) result[i] = deserialize(ctx, depth, item);
	}
...
function assignProperty(ctx, depth, object, key, value) {
	if (typeof key === "string") assignStringProperty(object, deserializeString(key), deserialize(ctx, depth, value));
	else {
		...
		switch (typeof actual) {
			case "string":
				...
				break;
			case "symbol":
				if (isValidSymbol(actual)) object[actual] = deserialize(ctx, depth, value);
				break;
...
function deserializeProperties(ctx, depth, node, result) {
	const keys = node.k;
	if (keys.length > 0) for (let i = 0, vals = node.v, len2 = keys.length; i < len2; i++) assignProperty(ctx, depth, result, keys[i], vals[i]);
	return result;
}
function deserializeObject(ctx, depth, node) {
	const result = assignIndexedValue(ctx, node.i, node.t === 10 ? {} : /* @__PURE__ */ Object.create(null));
...
function deserializeSet(ctx, depth, node) {
	...
	for (let i = 0, items = node.a, len = items.length; i < len; i++) result.add(deserialize(ctx, depth, items[i]));
	return result;
}
function deserializeMap(ctx, depth, node) {
	const result = assignIndexedValue(ctx, node.i, /* @__PURE__ */ new Map());
	for (let i = 0, keys = node.e.k, vals = node.e.v, len = keys.length; i < len; i++) result.set(deserialize(ctx, depth, keys[i]), deserialize(ctx, depth, vals[i]));
	return result;
}
function deserializeArrayBuffer(ctx, node) {
	if (node.s.length > MAX_BASE64_LENGTH) throw new SerovalMalformedNodeError(node);
	return assignIndexedValue(ctx, node.i, ARRAY_BUFFER_CONSTRUCTOR(deserializeString(node.s)));
}
function deserializeTypedArray(ctx, depth, node) {
	...
	return assignIndexedValue(ctx, node.i, new construct(source, offset, node.l));
}
function deserializeDataView(ctx, depth, node) {
	...
	const offset = (_a = node.b) != null ? _a : 0;
	if (offset < 0 || offset > source.byteLength) throw new SerovalMalformedNodeError(node);
	return assignIndexedValue(ctx, node.i, new DataView(source, offset, node.l));
}
...
function deserializeAggregateError(ctx, depth, node) {
	return deserializeDictionary(ctx, depth, node, assignIndexedValue(ctx, node.i, new AggregateError([], deserializeString(node.m))));
}
function deserializeError(ctx, depth, node) {
	const construct = deserializeKnownValue(node, ERROR_CONSTRUCTOR, node.s);
	return deserializeDictionary(ctx, depth, node, assignIndexedValue(ctx, node.i, new construct(deserializeString(node.m))));
}
...
function deserializeBoxed(ctx, depth, node) {
	return assignIndexedValue(ctx, node.i, Object(deserialize(ctx, depth, node.f)));
}
function deserializePlugin(ctx, depth, node) {
	...
	if (currentPlugins) {
		...
		for (let i = 0, len = currentPlugins.length; i < len; i++) {
			const plugin = currentPlugins[i];
			if (plugin.tag === tag) return assignIndexedValue(ctx, node.i, plugin.deserialize(node.s, new DeserializePluginContext(ctx, depth), { id: node.i }));
		}
	}
	throw new SerovalMissingPluginError(node.c);
}
function deserializePromiseConstructor(ctx, node) {
	const value = assignIndexedValue(ctx, node.i, assignIndexedValue(ctx, node.s, PROMISE_CONSTRUCTOR()).p);
...
function deserializeIteratorFactoryInstance(ctx, depth, node) {
	...
	return sequenceToIterator(deserialize(ctx, depth, node.a[1]));
}
function deserializeAsyncIteratorFactoryInstance(ctx, depth, node) {
	deserialize(ctx, depth, node.a[0]);
	return streamToAsyncIterable(deserialize(ctx, depth, node.a[1]));
}
function deserializeStreamConstructor(ctx, depth, node) {
	...
	const items = node.a;
	const len = items.length;
	if (len) for (let i = 0; i < len; i++) deserialize(ctx, depth, items[i]);
	return result;
}
...
function deserializeSequence(ctx, depth, node) {
	...
	for (let i = 0, len = node.a.length; i < len; i++) result.v[i] = deserialize(ctx, depth, node.a[i]);
	return result;
}
...
function serializeDictionary(ctx, node, init) {
	if (node.p) {
		...
		if (base.features & 8) init = serializeWithObjectAssign(ctx, node, node.p, init);
			...
			if (assignments) return "(" + assignIndexedValue2(ctx, node.i, init) + "," + assignments + getRefParam(ctx, node.i) + ")";
		}
	}
	return init;
}
...
function serializeRegExp(ctx, node) {
	if (ctx.base.features & 32) return "/" + node.c + "/" + node.m;
	throw new SerovalUnsupportedNodeError(node);
}
...
function serializePromiseConstructor(ctx, node) {
	...
	return "(" + assignIndexedValue2(ctx, node.s, getConstructor(ctx, node.f) + "()") + ").p";
}
...
function serializeIteratorFactory(ctx, node) {
	...
	result += assignIndexedValue2(ctx, node.i, "(" + SERIALIZED_ITERATOR_CONSTRUCTOR + ")(" + getRefParam(ctx, node.f.i) + ")");
...
function serializeAsyncIteratorFactory(ctx, node) {
	...
	if (result) result += ",";
	const iterator = assignIndexedValue2(ctx, node.i, "(" + SERIALIZED_ASYNC_ITERATOR_CONSTRUCTOR + ")(" + getRefParam(ctx, symbol.i) + "," + getRefParam(ctx, promise.i) + ")");
...
function serializeStreamConstructor(ctx, node) {
	...
	if (len) {
		...
		for (let i = 1; i < len; i++) values += "," + serialize(ctx, node.a[i]);
...
function serialize(ctx, node) {
	switch (node.t) {
		...
		default: return assignIndexedValue2(ctx, node.i, serializeAssignable(ctx, node));
	}
}
...
function parseAggregateError2(ctx, depth, id, current) {
	const options = getErrorOptions(current, ctx.base.features);
	return createAggregateErrorNode(id, current, options ? parseProperties2(ctx, depth, options) : NIL);
}
...
function handlePromiseSuccess(id, depth, data) {
	if (this.state.alive) {
		...
		if (parsed) onParse(this, createSerovalNode(23, id, NIL, NIL, NIL, NIL, NIL, [parseSpecialReference(this.base, 2), parsed], NIL, NIL, NIL, NIL));
...
function handlePromiseFailure(id, depth, data) {
	if (this.state.alive) {
		const parsed = parseWithError(this, depth, data);
		if (parsed) onParse(this, createSerovalNode(24, id, NIL, NIL, NIL, NIL, NIL, [parseSpecialReference(this.base, 3), parsed], NIL, NIL, NIL, NIL));
	}
	popPendingState(this);
}
...
function parseObjectPhase2(ctx, depth, id, current, currentClass) {
	switch (currentClass) {
		case Object: return parsePlainObject2(ctx, depth, id, current, false);
		case NIL: return parsePlainObject2(ctx, depth, id, current, true);
		case Date: return createDateNode(id, current);
		case Error:
		case EvalError:
		case RangeError:
		...
		case SyntaxError:
		case TypeError:
		case URIError: return parseError2(ctx, depth, id, current);
		case Number:
		case Boolean:
		case String:
		case BigInt: return parseBoxed2(ctx, depth, id, current);
		case ArrayBuffer: return createArrayBufferNode(ctx.base, id, current);
		case Int8Array:
		case Int16Array:
		case Int32Array:
		...
		case Uint8ClampedArray:
		case Float32Array:
		case Float64Array: return parseTypedArray2(ctx, depth, id, current);
		case DataView: return parseDataView2(ctx, depth, id, current);
		case Map: return parseMap2(ctx, depth, id, current);
		case Set: return parseSet2(ctx, depth, id, current);
		default: break;
	}
	if (currentClass === Promise || current instanceof Promise) return parsePromise2(ctx, depth, id, current);
	const currentFeatures = ctx.base.features;
	if (currentFeatures & 32 && currentClass === RegExp) return createRegExpNode(id, current);
	if (currentFeatures & 16) switch (currentClass) {
		case BigInt64Array:
		case BigUint64Array: return parseBigIntTypedArray2(ctx, depth, id, current);
		default: break;
	}
	if (currentFeatures & 1 && typeof AggregateError !== "undefined" && (currentClass === AggregateError || current instanceof AggregateError)) return parseAggregateError2(ctx, depth, id, current);
	if (current instanceof Error) return parseError2(ctx, depth, id, current);
	if (SYM_ITERATOR in current || SYM_ASYNC_ITERATOR in current) return parsePlainObject2(ctx, depth, id, current, !!currentClass);
	throw new SerovalUnsupportedTypeError(current);
}
function parseObject(ctx, depth, id, current) {
	...
	if (isSequence(current)) return parseSequence2(ctx, depth, id, current);
	const currentClass = current.constructor;
